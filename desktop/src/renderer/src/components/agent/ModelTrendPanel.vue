...
import type { DailyStats, HourlyUsageStats, ModelStats } from '@shared/models'

interface Props {
  dailyModelStats: DailyStats[]
  hourlyModelStats: HourlyUsageStats[]
...
const modelUsageTotals = computed(() => {
  ...
  const rows = props.singleDayRange ? props.hourlyModelStats : props.dailyModelStats
  rows.forEach((row) => {
    Object.entries(row.agentTokens || {}).forEach(([model, tokens]) => {
      const value = Number(tokens) || 0
      if (value > 0) totals.set(model, (totals.get(model) || 0) + value)
    })
  })

  if (totals.size === 0) {
    props.modelStats.forEach((model) => {
      if (model.totalTokens > 0) totals.set(model.model, model.totalTokens)
    })
  }
  return totals
})
...
const sortedModelNames = computed(() =>
  [...modelUsageTotals.value.entries()]
    .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
    .map(([model]) => model),
)

const legendItems = computed(() => [TOTAL_SERIES_KEY, ...sortedModelNames.value])
...
function legendLabel(key: string): string {
  return key === TOTAL_SERIES_KEY ? getTotalLabel() : key
}

function legendColor(key: string): string {
  return key === TOTAL_SERIES_KEY ? TOTAL_SERIES_COLOR : modelColor(key)
}
...
function renderChart(): void {
  ...
  if (props.dailyModelStats.length === 0 && !todayMode) {
    if (chart) {
      chart.dispose()
      chart = null
    }
  ...
  if (todayMode) {
    const hourlyStats = normalizeHourlyStats(props.hourlyModelStats)
  ...
  if (
    sortedModelNames.value.length === 0 &&
    props.dailyModelStats.every((day) => day.totalTokens <= 0)
  ) {
    chart.dispose()
    chart = null
    showEmpty(chartRef.value, tr('noModelDataWithTokens'))
    return
  }

  const dates = props.dailyModelStats.map((d) => d.date)
  const series = legendItems.value
    .filter((key) => legend.isSelected(key))
    .map((key) =>
      buildTrendSeries(
        key,
        props.dailyModelStats.map((day) =>
...
watch(
  () => [
    props.dailyModelStats,
    props.hourlyModelStats,
    props.modelStats,
...
<template>
  <article class="panel chart-panel model-trend-panel">
    <div class="panel-header with-token">
      <h3>{{ tr('modelUsageTrend') }}</h3>
    ...
    <div class="chart-legend">
      ...
      >
        <span class="legend-dot" :style="{ backgroundColor: legendColor(model) }"></span>
        <span>{{ legendLabel(model) }}</span>
      </button>
    </div>
