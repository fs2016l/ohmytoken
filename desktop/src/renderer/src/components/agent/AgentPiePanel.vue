...
function renderChart(): void {
  ...
  if (data.length === 0) {
    ...
    showEmpty(
      chartRef.value,
      legendItems.value.length === 0
        ? label('No agent data yet.', '暂无 Agent 数据。')
        : label('All agents are hidden.', '已隐藏全部 Agent。'),
    )
    return
  }

  const agentTotal = data.reduce((sum, item) => sum + item.value, 0)
  ...
  chart.on('click', (params: unknown) => {
    ...
    if (!name) return
    const agentKey = visibleAgents.find((agent) => (agentNames[agent] || agent) === name)
    if (agentKey) emit('openAgent', agentKey)
  })
}
